---
title: "Register Grammar — Matheme, Mytheme, Episteme"
aliases:
  - "register grammar"
...
source_ids:
  ...
  - detienne-vernant-1978-cunning-intelligence
tags:
...
There is no canonical quota of one fully developed matheme, mytheme, or episteme per station. Such a quota contradicts the relation the grammar serves: `1` is Poly, a real local articulation whose plurality does not cease to be one field. A station can therefore carry several instances in one register when each performs a necessary and non-duplicative operation.

The admission test remains **aletheia-work**. A carrier enters main prose when it derives, discloses, tests, remembers, turns, or completes something no admitted carrier performs at that landing. A carrier which only repeats a recognised pattern moves to a link, plate, inset, note, path, or deep-dive fixture. Richness is organised by functional hierarchy—governing movement, subordinate carriers, explicit transitions, and return—not by arbitrary scarcity. The reader who follows only the main text must still receive the whole argument; linked depths extend rather than repair it.

## Register as the mapping-and-return channel

Register is not a filing label that assigns one record to one bucket. It is the **admission-and-return channel** of the holographic piece: the mechanism by which polyvalent detail from across the field maps back into the sections with precision, and by which records that live outside the essay point back to exact essay locations. A movement or argument declares the **register composition** it admits and carries — a station can draw on matheme, mytheme, and episteme at once (the worked example below does exactly that) — and the depth files beside it declare their register boundary and their route of return to the precise essay block. That outward-and-return movement is what makes the whole perceptible from every local entrance; it is the same 4+2 circulation the essay itself performs, enacted by the vault rather than only described by it.
