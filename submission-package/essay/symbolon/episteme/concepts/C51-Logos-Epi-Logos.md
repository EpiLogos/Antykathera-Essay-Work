...
The September 15 authorial development gives this return a further exactness: **Objective Internality is paradigmatic existence as lived/enacted structure; Epi-Logos is the capacity of an achieved paradigm to place its own articulation back within the field of inquiry.** The paradigm is lived before it is represented. Epi-Logos begins when its pattern can become explicit without the explicit pattern being promoted to the whole of Life or World.
...
The emerging **Expression** surface belongs here as a technical consequence, not as a proof. If a paradigm is carried only as hidden schema, its relations remain available chiefly to the machinery that executes them. Rendering states and transitions through text, glyph, image, relation and living transformation can make the paradigm available to perception and manipulation as a whole expressive object. Its philosophical value depends on whether that expression remains connected to the sources, conditions and actions it represents and whether changing it can return to the paradigm's operation. A beautiful visualization which cannot alter the enacted field remains a picture of Epi-Logos, not its completion.
