...
6. `essay-workshop/THE-RETURN-OF-ZERO.md` is the sovereign manuscript. Generated `ROOM.md` files are compact authoring refractions; protected `READING.md` files are optional cross-source learning routes. Neither rooms nor retrieval adapters supersede canonical nodes.
...
The notation, determinations, and derivational operations in `essay-workshop/sources-texts-references/10-7-2026-core-theorems-pithy.md` and their designated supporting files are Frank's native QL language. They are not borrowed doctrines waiting to be validated by Jung, calculus, theology, music, topology, or any other neighbouring discourse. Those fields can refract, witness, or qualify a QL operation only after their distinct relation has been recovered.
